...
if($query->num_rows > 0){
    ...
    //set column headers
    $fields = array('Name', 'State', 'Q_No', 'Question', 'Answer', 'Message ID', 'Message', 'Company_id');  
    fputcsv($f, $fields, $delimiter);
    
    //output each row of the data, format line as csv and write to file pointer
    while($row = $query->fetch_assoc()){
      //  $status = ($row['status'] == '1')?'Active':'Inactive';
        $lineData = array($row['name'], $row['state'], $row['question_id'], $row['question'], $row['answer'], $row['message_id'], $row['message'], $row['company_id']);
